(function ($, dnn) {
    ...
    dnn.searchResult.defaultSettings = {
        ...
        tagsText: 'Tags :',
        addTagText: 'Ajouter un tag',
        resultsCountText: '{0} résultats trouvés',
        currentPageIndexText: 'Page actuelle :',
        linkTarget: '',
        cultureCode: 'fr-FR'
    };
        ...
        if (data && data.results && data.results.length) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                if (result.Results.length > 1) {
                    ...
                    // render descriptions for subsets
                    for (var j = 0; j < result.Results.length; j++) {
                        markup += '<div class="dnnSearchResultItem-Description">' + result.Results[j].Snippet + '</div>';
                    }
    ...
    dnn.searchResult.init = function (settings) {
        ...
        dnn.searchResult.searchInput = $('#dnnSearchResult_dnnSearchBox').dnnSearchBox({
            ...
            defaultText: "Entrer les termes recherchés",
            advancedText: 'Avancé',
            seemoreText: 'Afficher plus de résultats',
