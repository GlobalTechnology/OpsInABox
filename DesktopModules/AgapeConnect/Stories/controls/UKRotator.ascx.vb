...
Namespace DotNetNuke.Modules.AgapeConnect.Stories.controls
    Partial Class UKRotator
        Inherits Entities.Modules.PortalModuleBase
        'Adding Stories Translation
        Dim d As New StoriesDataContext
        Public PauseTime As Integer = 3000 ' milliseconds
        Public AnimTime As Integer = 1000 ' milliseconds
        Public divWidth As Integer = 625
        Public divHeight As Integer = 325
        ...
        Public Sub Initialize(ByVal Stories As System.Linq.IOrderedQueryable(Of AP_Stories_Module_Channel_Cache), settings As Hashtable)
            ...
            If (Not String.IsNullOrEmpty(settings("AnimSpeed"))) Then
                AnimTime = CInt(settings("AnimSpeed")) * 1000 'milliseconds
            End If

            Dim photoWidth As Integer = 625
            If Not String.IsNullOrEmpty(settings("PhotoWidth")) Then
                photoWidth = settings("PhotoWidth")
            End If

            Dim photoAspect As Double = 1.92
            If Not String.IsNullOrEmpty(settings("PhotoWidth")) Then
