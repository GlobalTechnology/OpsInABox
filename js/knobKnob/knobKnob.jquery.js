    ...
    $.fn.knobKnob = function (props) {
        ...
        return this.each(function () {
            ...
            if (options.value > 0 && options.value <= 359) {
                ...
                options.turn(currentDeg / 359, ID);

            }
            else if (options.volumesid != "") {

                var vol = $('#' + options.volumesid).val();
                
                var vols = vol.split(";")
                var thisVol = 0;
                for (var i = 0; i < vols.length; i++)
                    if (vols[i].indexOf(ID + "=") == 0) 
                       thisVol = vols[i].substring(vols[i].indexOf("=") + 1);
                    
rotation = currentDeg = (359 * thisVol / 30);
                knobTop.css('transform', 'rotate(' + (currentDeg) + 'deg)');
